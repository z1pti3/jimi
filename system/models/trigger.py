...
def failedTrigger(workerID,failureType,msg=""):
    ...
    if len(triggers) > 0:
        triggerWorkers = trigger._trigger().query(query={"workerID" : workerID})["results"]
        if len(triggerWorkers) > 0:
            ...
            if _class:
                ...
                if triggerClass:
                    ...
                    audit._audit().add("Error",failureType,{"type" : "systemEvent", "eventType" : failureType, "workerID" : workerID, "triggerID" : worker_["_id"], "triggerName" : worker_["name"], "msg" : msg})
